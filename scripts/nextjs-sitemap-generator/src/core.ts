...
class SiteMapper {
  ...
  buildPathMap (dir) {
    ...
    for (const site of data) {
      ...
      if (this.lastmodHashFile && !fs.statSync(nextPath).isDirectory()) {
        pathMap[pagePath].md5 = md5.sync(nextPath)
      }
    }
  ...
  async getSitemapURLs (dir) {
    ...
    if (this.lastmodHashFile) {
      const lastmodHashes = fs.existsSync(this.lastmodHashFile) ? require(this.lastmodHashFile) : {}
  ...
  async sitemapMapper (dir) {
    ...
    filteredURLs.forEach((url) => {
      ...
      if (this.lastmodHashFile) {
        const isHTML = fs.existsSync(`${dir}${url.pagePath}.html`)
        lastmods[url.pagePath] = {
          date: format(new Date(), 'yyyy-MM-dd'),
          md5: isHTML ? md5.sync(`${dir}${url.pagePath}.html`) : 'bleh'
        }
      }
    ...
    if (this.lastmodHashFile) {
      fs.writeFileSync(this.lastmodHashFile, JSON.stringify(lastmods))
    }
  }
}
